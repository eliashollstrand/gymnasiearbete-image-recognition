...
train_image_folder = os.path.join('datasets', 'dataset_train_images')
test_image_folder = os.path.join('datasets', 'dataset_test_images')
img_height, img_width = 250, 250  # size of images
num_classes = len(os.listdir("datasets/dataset_train_images"))
name = "face_classifier_v4"
...
checkpoint = ModelCheckpoint(f"models/{name}.h5",
                             monitor="val_loss",
                             mode="min",
                             save_best_only=True,
                             verbose=1)
